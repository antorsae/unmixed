...
const { listAudioFiles, getWindowRmsLevels } = require('./audio-analysis-utils');
...
async function analyzeFile(filePath) {
  ...
  // Try astats first
  let levels = await getWindowRmsLevels(filePath, WINDOW_MS, {
...
async function main() {
  ...
  console.log('='.repeat(80));
  console.log('');
  console.log('Track                   | p10 (Noise) | p90 (Signal) | Gap     |');
  console.log('------------------------|-------------|--------------|---------|');
  ...
  for (const r of results) {
    ...
    console.log(`${name} | ${noise} | ${signal} | ${gap} |`);
  }
  ...
  console.log(`
📊 CLUSTER ANALYSIS:
   Noise floor range (p10): ${Math.min(...noiseFloors).toFixed(1)} to ${Math.max(...noiseFloors).toFixed(1)} dB
   Signal level range (p90): ${Math.min(...signalLevels).toFixed(1)} to ${Math.max(...signalLevels).toFixed(1)} dB

   Average noise floor (p10): ${avgNoiseFloor.toFixed(1)} dB
   Average signal level (p90): ${avgSignalLevel.toFixed(1)} dB
